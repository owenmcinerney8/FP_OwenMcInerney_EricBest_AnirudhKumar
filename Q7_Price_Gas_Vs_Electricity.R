...
fuel_df_final <- fuel_df %>%
  reframe(date = date, miles_per_dollar_gas = gas_avg_mpge/gas, 
          miles_per_dollar_electricity = electricity_avg_mpge/electricity)

fuel_df_final <- fuel_df_final %>%
  pivot_longer(cols = c(miles_per_dollar_electricity, miles_per_dollar_gas), 
               names_to = 'fuel_type', names_prefix = 'miles_per_dollar_', values_to = 'miles_per_dollar') 
...
fuel_plot <- fuel_df_final %>% 
  ...
  labs(title = "Miles per Dollar by Month: Electricity Vs Gas", 
       subtitle = "Using 2024 vehicles and 2022 monthly fuel prices", x = "Month", y = "Miles per Dollar", color = "Fuel Type") +
  theme_bw()
